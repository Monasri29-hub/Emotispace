import React from 'react';

const BrainCircuitIcon: React.FC<React.SVGProps<SVGSVGElement>> = (props) => (
  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}>
    <path d="M12 5a3 3 0 1 0-5.993.142" />
    <path d="M12 5a3 3 0 1 1 5.993.142" />
    <path d="M15 13a3 3 0 1 0-5.993.142" />
    <path d="M15 13a3 3 0 1 1 5.993.142" />
    <path d="M9 13a3 3 0 1 0-5.993.142" />
    <path d="M9 13a3 3 0 1 1 5.993.142" />
    <path d="M6.007 13.142A3 3 0 1 0 9 13" />
    <path d="M17.993 13.142A3 3 0 1 0 15 13" />
    <path d="M6.007 5.142A3 3 0 1 0 9 5" />
    <path d="M17.993 5.142A3 3 0 1 0 15 5" />
    <path d="M12 8v4" />
    <path d="M12 17v-4" />
    <path d="m15.5 10.5 2.5-1" />
    <path d="m6 9.5 2.5 1" />
    <path d="m15.5 15.5-2.5 1" />
    <path d="m6 16.5 2.5-1" />
  </svg>
);

export default BrainCircuitIcon;
